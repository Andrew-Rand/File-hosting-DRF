...
def error_handler(exc: Exception, context) -> Response:
    ...
    if isinstance(exc, BadRequestError):
        return Response(create_std_response(status_code=status.HTTP_400_BAD_REQUEST))

    elif isinstance(exc, exceptions.AuthenticationFailed):
        return Response(create_std_response(status_code=status.HTTP_401_UNAUTHORIZED))
